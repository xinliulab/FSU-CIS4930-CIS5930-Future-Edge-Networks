...
% Teaching note (90 s): before showing the answer, ask the class to look back at
% the GPRS diagram and find the two boxes that are missing from the EPC diagram.
% Let them notice the disappearance themselves.
\begin{frame}{Wait --- where did SGSN and GGSN go?}
  \gonebox
    ...
    {4G cut each box along that seam.

     \vspace{0.4em}
     MME decides and never touches your packets. S-GW and P-GW
     move packets and decide nothing.

     \vspace{0.4em}
     Now each side grows on its own.}
...
% Teaching note (60 s): the MSC has been on every diagram since 2G. Point at the
...
\begin{frame}{And the MSC? It had been there since 2G.}
  \gonebox
    ...
    {LTE carried IP and nothing else.
     ...
     equipment, its own staff, and its own bill.}
    {Nothing, at first --- and that was the problem.

     \vspace{0.4em}
     Voice had to stop being a network service and become an
     application riding on IP, like any other app.}

  \vspace{0.6em}
  \ask{So who carries your voice call on an all-IP network?}
\end{frame}
...
\begin{frame}{You have now watched the same move twice}
  \vspace{0.3em}
  \centering
  \begin{tikzpicture}[scale=0.82,transform shape,
      one/.style={draw=softgray,fill=black!4,rounded corners=3pt,
        minimum width=2.3cm,minimum height=0.8cm,align=center,font=\small\bfseries}]

    \node[one] (bs) at (0,1.4) {Base station};
    \node[netnode,minimum width=1.5cm] (bts) at (3.6,1.85) {BTS};
    \node[netnode,minimum width=1.5cm] (bsc) at (3.6,0.95) {BSC};
    \draw[flow] (bs) -- (bts);
    \draw[flow] (bs) -- (bsc);
    \node[font=\scriptsize,anchor=west,text width=4.1cm,align=left] at (4.9,1.4)
      {radio timing vs.\ call control};

    \node[one] (gprs) at (0,-1.0) {SGSN / GGSN};
    \node[corenode,minimum width=1.5cm] (mme) at (3.6,-0.55) {MME};
    \node[corenode,minimum width=1.8cm] (gw) at (3.6,-1.45) {S-GW / P-GW};
    \draw[flow] (gprs) -- (mme);
    \draw[flow] (gprs) -- (gw);
    \node[font=\scriptsize,anchor=west,text width=4.1cm,align=left] at (4.9,-1.0)
      {decisions vs.\ packet volume};

    \node[font=\small\bfseries,anchor=east,text=softgray] at (-1.4,1.4) {2G};
    \node[font=\small\bfseries,anchor=east,text=softgray] at (-1.4,-1.0) {4G};
  \end{tikzpicture}

  \vspace{0.8em}
  \takeaway{One box becomes two when its halves need to change at different speeds.}

  \vspace{0.3em}
  {\small\textcolor{softgray}{On Wednesday you will watch it happen twice more ---
   to the core, and then to the base station itself.}\par}
\end{frame}
