...
\usetikzlibrary{shapes.symbols,shapes.geometric,calc}
...
% ---------------------------------------------------------------------------
% Class 9 (RANBooster) additions. The authors' own deck puts one claim per slide in
% a dark red; \headline reproduces that, and nothing else in this deck uses the colour.
% ---------------------------------------------------------------------------

\definecolor{talkred}{RGB}{163,26,52}
\newcommand{\headline}[1]{{\color{talkred}\bfseries #1}}

% A radio unit, drawn the way the talk draws it: grey box + radiating arcs.
% #1 = node name, #2 = (x,y), #3 = label
\newcommand{\runode}[3]{%
  \node[draw=softgray,fill=gray!14,rounded corners=2pt,minimum width=1.1cm,
        minimum height=0.72cm,font=\bfseries\small] (#1) at #2 {#3};
  \foreach \rr in {0.16,0.28,0.40}{%
    \draw[softgray,thin] ([shift={(\rr,0)}]#1.east) arc (0:55:\rr);
    \draw[softgray,thin] ([shift={(\rr,0)}]#1.east) arc (0:-55:\rr);
  }%
}

% Node styles shared by every packet-flow diagram in this deck.
\tikzset{
  dunode/.style={draw=softgray,fill=white,minimum width=1.3cm,
    minimum height=0.75cm,font=\bfseries\small},
  mbox/.style={draw=softgreen,fill=softgreen,text=white,rounded corners=2pt,
    minimum height=0.72cm,align=center,font=\bfseries\scriptsize},
  ricnode/.style={draw=softgreen,fill=softgreen,text=white,rounded corners=2pt,
    minimum width=2.4cm,minimum height=0.62cm,font=\bfseries\small},
  pkt/.style={draw=softgray,fill=yellow!55,minimum width=0.5cm,
    minimum height=0.38cm,inner sep=1pt,font=\tiny\bfseries},
  pktb/.style={draw=softgray,fill=softgreen!35,minimum width=0.5cm,
    minimum height=0.38cm,inner sep=1pt,font=\tiny\bfseries},
  wire/.style={draw=softgray,thick}
}
