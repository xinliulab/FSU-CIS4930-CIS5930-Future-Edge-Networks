...
% SECTION TIMING: 41:00--54:00 (7 frames; the tour-map frame and the peak explainer
% are short beats, ~1 min each).
...
% Inserted 2026-09-09. The back half of this deck is five papers, and the seams
% between them read as unexplained jumps. This frame announces the route once;
% a one-slide reprise (\tourmap) marks each later stop. Say the rule out loud:
% every remaining topic is the SAME trade-off carried somewhere new.
\begin{frame}{Five places the curve must survive}
  \vspace{0.35em}
  {\small So far the trade-off lives on a whiteboard. The rest of today takes it
   into the field --- five stops, and at every one the question is the same:
   \textbf{where on the curve do you stand, and who gets to decide?}\par}
  ...
  \vspace{1.0em}
  \takeaway{One curve, five stops.\\When a new topic appears, find it on this map first.}
\end{frame}
