...
% SECTION TIMING: 32:00--60:00 (17 frames). Question 2 of the roadmap frame.
...
% OFDM primer, 16 frames (2026-09-13, third pass, at the author's request: one idea per
...
%   C. the FFT                      C1 two machines, C2 the 2x2 with numbers, C3 the 4x4,
%                                   C4 why "fast" matters
...
\begin{frame}{Question 2: which signal do we actually use?}
  ...
  {\small Before we can read OFDM as a radar we need to understand it properly. Most
   of you have only heard the name, so we build it from the ground up, in four parts:\par}
  \vspace{0.2em}
  {\small
  \begin{enumerate}\itemsep0pt
    ...
    \item Why the FFT --- the machine that adds waves up and takes them apart.
    \item Putting it together: transmitter, receiver, and the \emph{grid}.
  \end{enumerate}}

  \vspace{0.15em}
  \takeaway{By the end of part 4 you will have built the grid this whole lecture is about.}
\end{frame}
...
\begin{frame}{Bits pick the point}
  ...
  \vspace{0.2em}
  \takeaway{One wave carries 4 bits every $T$: slow.\\Part 2: send much more at the same time.}
\end{frame}
...
\begin{frame}{Why not one fast wave? Echoes.}
  ...
  \vspace{0.25em}
  \takeaway{Echoes are why we send many slow waves instead of one fast one.\\(Keep that sentence: later today the echo becomes the radar.)}
\end{frame}
...
\begin{frame}{The perpendicular family: frequencies $k/T$}
  ...
  \vspace{0.3em}
  \takeaway{We now know \emph{which} 64 waves to use.\\Part 3: the machine that adds them up and takes them apart.}
\end{frame}
...
\begin{frame}{Part 3: we need two machines}
  ...
  \vspace{0.3em}
  \takeaway{That table has a name: the FFT matrix. Let us look at a tiny one.}
\end{frame}
...
\begin{frame}{Why ``Fast'' is the important letter}
  ...
  \vspace{0.7em}
  {\small A 5G phone runs this for every symbol, tens of thousands of times a second,
   on a battery. With the plain table it could not. The FFT algorithm dates from 1965;
   the first paper proposing OFDM built on it is from 1971. That is not a
   coincidence.\par}

  \vspace{0.3em}
  \takeaway{OFDM is practical \emph{because} the FFT is fast.\\Part 4: put the pieces together.}
\end{frame}
...
\begin{frame}{Symbol after symbol: the grid}
  ...
  \vspace{0.2em}
  \takeaway{Question 2 answered: OFDM is a grid of cells, one wave per cell.\\Next slide: the same grid, read as a radar.}
\end{frame}
