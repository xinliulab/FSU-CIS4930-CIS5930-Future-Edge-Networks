...
\begin{frame}{802.11ax: dense efficiency, not peak}
  ...
  \vspace{0.1em}
  % Reconciles coloring with the CSMA/CA slide's "never talk over anyone":
  % "faint" means too weak to collide at my receiver, so deferring was needless.
  {\scriptsize\textcolor{softgray}{No contradiction with CSMA/CA: ``faint'' = too
    weak to collide at my receiver --- coloring skips a needless deferral.}\par}

  \vspace{0.2em}
  \takeaway{New target: dense efficiency, not a bigger single-link headline.}

  \glossline{HE = High Efficiency \quad RU = Resource Unit \quad BSS = Basic Service
    Set \quad cell = one AP's coverage (a BSS) \quad TWT = Target Wake Time \quad
    UL = uplink \quad coloring = a per-cell tag on frames}
